...
export function AccountCard({ account, pending, onEdit, onToggle, onDelete, onRefresh, onSync, onExport, onRelogin }: { account: AuthAccount; pending: boolean; onEdit: () => void; onToggle: () => void; onDelete: () => void; onRefresh: () => void; onSync: () => void; onExport: () => void; onRelogin: () => void }) {
  ...
  return <article className={cardClass} aria-label={`${account.label} 认证账号`}>
    ...
    <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border pt-4">{invalid ? <button onClick={onRelogin} disabled={pending} className="col-span-2 action-primary justify-center"><KeyRound size={15} />重新登录</button> : null}<button onClick={onRefresh} disabled={pending} className="action-secondary justify-center px-2 py-2 text-xs">{pending ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />}刷新令牌</button><button onClick={onSync} disabled={pending} className="action-secondary justify-center px-2 py-2 text-xs"><RotateCw size={14} />同步模型</button>{!isKimi && <button onClick={onExport} disabled={pending} className="action-secondary justify-center px-2 py-2 text-xs"><Download size={14} />导出 JSON</button>}</div>
